...
from astro_cubes.tools import find_nearest, moments, mc_errors
from src.utils.tables import cosmo, nu_cii
from src.utils.line_models import gaussian, gaussian_double
from src.utils.line_models import log_prior_single, log_probability_single, log_likelihood_single
from src.utils.line_models import log_prior_double, log_probability_double, log_likelihood_double
from src.utils.formulae import SFR_lagache, SFR_de_looze, SFR_de_looze_highz
...
class cristal_cube:
    ...
    def get_beam_aperture(self, x=0, y=0):
...
class cristal_spec:
    ...
    def get_snr(self):
        # Select line region
        i_min, i_max = self.get_line_limits(factor=1)
        line_region = np.arange(i_min, i_max, 1, dtype=int)
        signal = np.nansum(self.flux[i_min:i_max])

        # Select continuum region
        cont_region1 = np.arange(0, i_min, 1, dtype=int)
        cont_region2 = np.arange(i_max, len(self.vel)-1, 1, dtype=int)
        cont_region = np.concatenate([cont_region1, cont_region2])
        noise = self.flux[cont_region].std()

        n_line_pixels = i_max - i_min  # Peak-to-peak width
        S_N = signal / (noise * n_line_pixels ** 0.5)

        print(signal, noise, n_line_pixels, S_N)
    ...
    def fit_gauss_emcee(self, bounds, n_walkers=100, verbose=True):
        ...
        sampler.reset()
        # perform MCMCprint
        pos, prob, state = sampler.run_mcmc(pos, 1250)
        tau = sampler.get_autocorr_time(tol=0)  # tol=0 for stricter convergence checks
    ...
    def fit_gauss_broad_emcee(self, bounds, n_walkers=200, verbose=True):
        ...
        pos = [pos_min + psize *rng.random(ndim) for i in range(nwalkers)]
        print(pos[0])
        ...
        sampler.reset()
        # perform MCMCprint
        pos, prob, state = sampler.run_mcmc(pos, 7500)
        tau = sampler.get_autocorr_time(tol=0)  # tol=0 for stricter convergence checks
        print(f"Autocorrelation time: {tau}")
        samples = sampler.flatchain
...
def make_cristal_cube(file, ID, z, ra, dec, fwhm_cii):
    ...
    Cube = cristal_cube(nu=nu, vel_axis=vel_axis, cube=cube, wcs=wcs, header=header, ID=ID,
                        pix_scale=pix_scale, z=z, beams=beams, ra=ra, dec=dec, fwhm_cii=fwhm_cii)

    return Cube

##### THE TWO FUNCTIONS BELOW COULD PROBABLY BE FOLDED INTO THE CRISTAL_CUBE CLASS

def make_cristal_cube_cutout(Cube, size, centre):
    """size in arcsec"""
    ...
    Cube_cutout = cristal_cube(nu=Cube.nu, vel_axis=Cube.vel_axis,
...
def mask_cristal_cube(Cube, c_mask, r_mask):
    """size in arcsec"""
    y_mask, x_mask = c_mask.to_pixel(wcs=Cube.wcs)
    x_mask, y_mask = np.array(x_mask, dtype=int), np.array(y_mask, dtype=int)

    cube_masked = Cube.cube.copy()
    for i in range(len(x_mask)):
        cube_masked[:, x_mask[i] - r_mask[i]:x_mask[i] + r_mask[i], y_mask[i] - r_mask[i]:y_mask[i] + r_mask[i]] = 0

    Cube_masked = cristal_cube(nu=Cube.nu, vel_axis=Cube.vel_axis,
                               cube=cube_masked, wcs=Cube.wcs, ID=Cube.ID,
                               pix_scale=Cube.pix_scale, z=Cube.z, beams=Cube.beams,
